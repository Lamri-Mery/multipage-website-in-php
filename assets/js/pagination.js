$(function(){
	...
	var numberOfImages = $('#loop .goldbook').length;	//crée une variable numberOfImages qui sélectionne l'élément avec la classe list-img dans l'élément avec l'id loop et conte les
	//console.log(numberOfImages);
	var limitPerPage = 3;
	$('#loop .goldbook:gt(' + (limitPerPage - 1) +')').hide();	//:gt() signifie que l'Éléments dont l'index est supérieur à (greater than) l'index spécifié
	...
	$('.pagination li.current-page').on('click', function() {
		...
		if($(this).hasClass('active')){
			...
			for (var i = grandTotal - limitPerPage; i < grandTotal; i++) {
				$('#loop .goldbook:eq(' + i + ')').show();
			}
		}	
	})
	/***faire mon action quand je clique sur le boutton suivant & précédent***/
